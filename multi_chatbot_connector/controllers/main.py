...
class LivechatMultiController(LivechatController):
    ...
    def get_session(self, channel_id, anonymous_name, mail_chat=None, **kwargs):
        ...
        if request.website.user_id != c_partner:
            res.update({'user_id':c_partner.id,
                        'user_name':c_partner.name,
                        'user_login':c_partner.login,})
        return res
    ...
    def feedback(self, uuid, rate, reason=None, **kwargs):
        ...
        if channel and channel.helpdesk_lead_id:
            ...
            c_partner.sudo().write({'is_busy':False})
    ...
    def IssueCategory(self, **kwargs):
        ...
        c_user = request._context.get('uid') or request._uid or request.env.user.id
        c_user = request.env['res.users'].sudo().search([('id','=',c_user)],limit=1)
        livechat_channel = request.env['im_livechat.channel'].sudo().search([('issue_category','in',online_help_category.ids),('company_id','=',c_user.company_id.id)])
    ...
    def LeadSubmit(self, uuid, helpdesk_info, **kwargs):
        ...
        c_user = request._context.get('uid') or request._uid or request.env.user.id
        c_user = request.env['res.users'].sudo().search([('id','=',c_user)],limit=1)
        livechat = Im_Livechat.sudo().search([('issue_category','=',int(issue_category)),('company_id','=',c_user.company_id.id)])
        if uuid and Channel_id and livechat:
            Channel_id.sudo().write({'livechat_channel_id':livechat.id})
            available = Channel_id.livechat_channel_id.get_available_users()
            if issue_category:
                ...
                Channel_id.sudo().write({'channel_partner_ids':[(6, 0, available_users.ids)]})
            ...
            l_list = []
            if helpdesk_id and helpdesk_id.maill_channel_id and helpdesk_id.maill_channel_id.livechat_channel_id and helpdesk_id.maill_channel_id.livechat_channel_id.mailing_list_id:
                l_list.append(helpdesk_id.maill_channel_id.livechat_channel_id.mailing_list_id.id)
