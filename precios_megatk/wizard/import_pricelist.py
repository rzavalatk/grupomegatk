...
class ImportInventory(models.TransientModel):
    ...
    def action_import(self):
        ...
        if not self.data:
            raise exceptions.UserError(_("Seleccione un archivo!"))
        ...
        try:
            ...
            raise exceptions.UserError(_("Archivo no valido!"))
        keys = reader_info[0]
        if not isinstance(keys, list) or ('code' not in keys):
            raise exceptions.UserError(_("No se encontraron 'códigos' de productos"))
        del reader_info[0]
        values = {}
        for i in range(len(reader_info)):
            ...
            if len(tmp_val) > 1:
                raise exceptions.UserError(_("Existen productos con codigos repetidos"))
            if tmp_val:
                vals = {
