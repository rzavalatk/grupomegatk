...
class PrintProductLabel(models.TransientModel):
    ...
    name = fields.Char(default='Imprimir etiquetas de productos')
    message = fields.Char(readonly=True)
    output = fields.Selection(selection=[('pdf', 'PDF')], string='Imprimir en', default='pdf') 
    mode = fields.Selection(
        selection=[('product.product', 'Products')],
        help='Campo técnico para especificar el modo del asistente de impresión de etiquetas.',
    ...
    humanreadable = fields.Boolean(
        string='Código de barras legible por humanos',
        default=False,
    )
    ...
    company_id = fields.Many2one(
        ...
        help='Especifique una empresa para las etiquetas de los productos.'
    )
    ...
    def _compute_is_template_report(self):
        for wizard in self:
            # flake8: noqa: E501
            wizard.is_template_report = self.report_id == self.env.ref('barcode_product_label.action_report_product_label_from_template')

    def get_labels_to_print(self):
        self.ensure_one()
        labels = self.label_ids.filtered(lambda l: l.selected and l.qty)
        if not labels:
            raise UserError(_('Nada para imprimir, establecer la cantidad de etiquetas en la tabla.'))
        return labels
    ...
    def _prepare_report(self):
        ...
        if not self.report_id:
            raise UserError(_('Seleccione un tipo de etiqueta.'))
