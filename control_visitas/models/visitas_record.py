    ...
    def agrupar_registros(self):
        ...
        if self.fecha_final and self.fecha_reporte:
            if self.fecha_final < self.fecha_reporte:
                ...
                visitas = self.env['control.visitas'].sudo().search([('fecha', '=', self.fecha_reporte),('region', 'in', ['TGU', 'SPS'])])
            else:
                 visitas = self.env['control.visitas'].sudo().search([('fecha', '>=', self.fecha_reporte),('fecha', '<=', self.fecha_final),'|',('region', '=','TGU'),('region','=','SPS')])
