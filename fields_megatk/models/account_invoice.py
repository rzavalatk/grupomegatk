...
class Account_Move(models.Model):
    ...
    def _check_team_id(self):
        """Override la constraint para permitir team_id vacío en asientos que no sean facturas de venta"""
        for record in self:
            # Solo valida si es factura de venta (out_invoice)
            if record.move_type == 'out_invoice' and not record.team_id:
                # Aquí podríamos lanzar error si queremos, pero lo dejamos pasar
                pass
    invoice_payment_term_id = fields.Many2one('account.payment.term', string='Payment Terms',
        check_company=True,
        readonly=False, required=False,)
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if term_id:
                ...
                if term.line_ids and any(line.nb_days > 0 for line in term.line_ids):
...
class AccountMoveLine(models.Model):
    ...
    def _compute_ref_pago(self):
        ...
        payments_refs = self.env['account.payment'].search([
            ('memo', 'in', self.mapped('move_id.name'))
        ])

        # Crear un diccionario de referencias de pago y fechas de pago para facilitar el acceso
        payments_data = {payment.memo: (payment.name, payment.date) for payment in payments_refs}
